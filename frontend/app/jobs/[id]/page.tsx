...
async function getJobDetail(id: string) {
  ...
  const job = jobs.find((item: any) => item.id === id)
  ...
  return (
    <main className="p-8 space-y-6">
      ...
      <div className="border rounded-lg p-6 space-y-2">
        <p><strong>Case Name:</strong> {job.caseName}</p>
        <p><strong>Duration:</strong> {job.duration} minutes</p>
        <p><strong>Status:</strong> {job.status}</p>
        <p><strong>Location:</strong> {job.location}</p>
        <p><strong>Reporter:</strong> {job.reporter?.name || '-'}</p>
        <p><strong>Editor:</strong> {job.editor?.name || '-'}</p>
      </div>
