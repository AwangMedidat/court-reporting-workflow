        ...
        {jobs.map((job) => (
          <tr key={job.id}>
            ...
            <td className="space-x-2">
              {job.status === 'NEW' && (
                <button onClick={() => handleAssignReporter(job.id)} className="bg-green-600 text-white px-3 py-1 rounded">
                Assign Reporter
              </button>
              )}
              {job.status === 'ASSIGNED' && (
                <button onClick={() => handleTranscribed(job.id)} className="bg-yellow-500 text-white px-3 py-1 rounded">
                  Mark Transcribed
                </button>
              )}
              {job.status === 'TRANSCRIBED' && (
                <button onClick={() => handleAssignEditor(job.id)} className="bg-purple-600 text-white px-3 py-1 rounded">
                  Assign Editor
                </button>
              )}
              {job.status === 'REVIEWED' && (
                <button onClick={() => handleComplete(job.id)} className="bg-gray-800 text-white px-3 py-1 rounded">
                  Complete Job
                </button>
              )}
            </td>
